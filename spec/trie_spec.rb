...
describe 'empty trie' do
  ...
  it 'should not get or has_key?' do
    ...
    expect(@trie.key?('anything')).to be false
  end
...
describe 'non-empty trie' do
  ...
  it 'should has_key? keys it has' do
    expect(@trie.key?('Hello')).to be true
    expect(@trie.key?('Hello, brother')).to be true
    expect(@trie.key?('Hello, bob')).to be true
  end

  it "should not has_key? keys it doesn't have" do
    expect(@trie.key?('Nope')).to be false
  end
  ...
  it 'should match wildcards' do
    expect(@trie.wildcard('H*ll.')).to eql(%w[Hello Hilly])
